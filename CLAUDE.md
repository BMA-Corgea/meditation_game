...
This project is a small React/Vite meditation toy that stages distraction rather than challenge. The player sits inside a loop of tempting cards, fake decisions, ambient interruptions, fixation, and a large meaningless total. The constant alternative is `Breathe`, which briefly clears the screen without turning calm into a score system.
...
- A persistent `Breathe` card sits in the hand at all times and must remain visible and playable.
...
- The app now tracks two hidden pacing pressures:
  - `settleLevel`, which rises through breathing and sustained non-interaction
  - `fixationLevel`, which rises through repeated engagement and craving loops
- `steady` and `jittery` cards stay in the row.
- `hovering`, `elusive`, and `centralizing` cards roam above it.
- `centralizing` cards drift toward the middle, render larger, glow harder, and linger longer.
...
- Clicking `Breathe` clears active effects, removes modals, dismisses walkers, fades out most cards, reduces fixation, and puts the app into a short calm state before normal clutter resumes.
...
- Vitest + Testing Library for integration coverage
...
There is also a `draw` interruption type for the transient whiteboard interaction.
...
- Idle settle triggers after `12000ms` of non-engagement.
- Non-breathe card cadence starts around `2800ms` and accelerates as fixation rises.
- Active card capacity starts at 5 and increases with fixation.
- Cards live for roughly `9000ms` to `16000ms`, then stretch longer under higher fixation.
...
- `Breathe` must remain visually protected from other cards, especially `centralizing` ones.
...
- `fixationLevel` should feel like attentional capture, not morality or failure.
...
- unit tests with Node's test runner for content, generator invariants, and fixation-driven card behavior
- integration tests with Vitest + Testing Library for UI interactions, ambience controls, and idle-settle behavior
- a Playwright smoke harness for the running app shell, including `Breathe` and ambience controls
